    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid" id='createhotel'>
                <h6 class="text-center display-4">اضافة دور  </h6>
                <form method="POST" enctype="multipart/form-data" id="updaterole">
                    @csrf
                    <div class="row">
                                {{-- name  --}}
                                <div class="col-12">
                                    <div class="form-group">
                                        <label>  اضافة دور  </label>
                                        <div class="input-group input-group-lg">
                                            <input type="text"   name='name'  class="form-control form-control-lg" value="{{$role->name}}" >
                                            <input type="hidden"   name='id'  class="form-control form-control-lg" value="{{$role->id}}" >
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-12 col-12">
                                </div>
                                 <label style="color:black; font-size:18px; margin-right:20px;"> الصلاحيات  </label>
                                 <div class="col-md-12 col-12"> </div> 
                                                @foreach($permission as $permission)   
                                                <?php $i=0; ?> 
                                            <div class="row"> 
                                                @foreach($rolePermissions as $ownrole)
                                                @if($permission->id == $ownrole )
                                                <?php $i=1; ?> 
                                                <div class="form-group" style="">
                                                        <div class="" style="font-size:19.5px;">
                                                        <label class="script" style="color:black;margin-right:109px;">{{$permission->name}} 
                                                        <input type="checkbox" name="permissions[]" value="{{ $permission->id}}" multiple checked>
                                               ...
                                               @endforeach  
                                                ...
                                                <div class="form-group" style="">
                                                        <div class="" style="font-size:19.5px;">
                                                        <label class="script" style="color:black;margin-right:109px;">{{$permission->name}} 
                                                        <input type="checkbox" name="permissions[]" value="{{ $permission->id}}" multiple>
                                                        <span class="w3docs"></span> 
                                                        </label>  
                                                        </div>          
                                               </div>
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response) {
                        ...
                        window.location.href='{{ route('allroles')}}';
                    }
                },
